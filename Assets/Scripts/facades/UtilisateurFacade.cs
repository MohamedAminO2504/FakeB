...
public class UtilisateurFacade : MonoBehaviour
{
    ...
    public IEnumerator Post(UtilisateurApi obj, System.Action onSuccess, System.Action<string> onError)
    {
        string jsonData = JsonUtility.ToJson(obj);
        Debug.Log(jsonData);
        byte[] jsonToSend = new UTF8Encoding().GetBytes(jsonData);

        using (UnityWebRequest request = new UnityWebRequest($"{parametter.GetUrl()}/utilisateur/connexion", "POST"))
        {
